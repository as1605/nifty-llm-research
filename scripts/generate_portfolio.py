...
async def generate_portfolio():
    ...
    try:
        ...
        historical_baskets = (
            await async_db[COLLECTIONS["baskets"]]
            .find()
        ...
        save_path = (
            Path(settings.data_dir) / "visualizations" / "portfolio_performance.png"
        )
        save_path.parent.mkdir(parents=True, exist_ok=True)

        plotter.plot_portfolio_performance(historical_baskets, str(save_path))
        ...
        logger.exception(f"Error generating portfolio: {e}")
        raise
